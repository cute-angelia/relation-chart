...
function getIndexByRoleId(role_id = "", data) {
  for (let n = 0; n < data.nodes.length; n++) {
    const element = data.nodes[n];
    if (element.role_id == role_id) {
      return n
    }
  }

  console.error("未发现" + role_id)
  return -1
}
...
export default class RelationChart {
  ...
  constructor(selector, data, configs = {}) {
    ...
    try {
      if (document.querySelectorAll(".svgclass")[0] != undefined) {
        document.querySelectorAll(".svgclass")[0].remove()
      }
    } catch (error) {
      console.log(error);
    }
    ...
    // relations
    data.links = []
    for (let n = 0; n < data.relations.length; n++) {
      const element = data.relations[n];
      let temp = {
        source: getIndexByRoleId(element.source, data),
        target: getIndexByRoleId(element.target, data),
    ...
    // console.log(d3.select(selector).remove())
